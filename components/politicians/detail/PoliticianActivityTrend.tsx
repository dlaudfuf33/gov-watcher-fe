  ...
  useEffect(() => {
    ...
    const drawChart = () => {
      ...
      for (let i = 0; i < billsData.length; i++) {
        const x =
          padding + ((width - padding * 2) / (billsData.length - 1)) * i;
        const y =
          padding + (height - padding * 2) * (1 - billsData[i] / maxValue);
        if (i === 0) {
          ctx.moveTo(x, y);
        } else {
          ctx.lineTo(x, y);
      ...
      for (let i = 0; i < speechesData.length; i++) {
        const x =
          padding + ((width - padding * 2) / (speechesData.length - 1)) * i;
        const y =
          padding + (height - padding * 2) * (1 - speechesData[i] / maxValue);
        if (i === 0) {
          ctx.moveTo(x, y);
        } else {
          ctx.lineTo(x, y);
